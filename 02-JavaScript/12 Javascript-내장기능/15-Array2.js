...
// arr.indexOf(item, from)는 인덱스 from부터 시작해 item을 찾는다.
...
// arr.includes(item, from)은 인덱스 from부터 시작해 item이 있는지를 검색해보는데, 해당하는 요소를 발견하면 true를 반환한다.
...
// 요소의 위치를 정확히 알고 싶은게 아니고 요소가 배열 내 존재하는지의 여부만 확인하고 싶다면, arr.includes를 사용하는게 좋다.
...
// 주어진 판별함수를 만족하는 첫 번째 값을 반환한다. 못찾으면 undefined를 반환한다.
// 찾고자 하는 항목이 아닌 검색 규칙을 구현한 콜백함수를 전달해야 한다.
const found = arr3.find(function(v) {
    // 파라미터로 전달되는 v는 배열의 모든 원소가 순차적으로 전달된다.
    console.log(v); // 5 12 8 131 44가 순차적으로 출력됨

    // v를 우리가 원하는 조건에 충족하는지 검사하여  true/ false를 리턴
    // true를 리턴하는 순간 배열의 탐색을 종료한다. (break와 동일한 기능)
    if(v % 2 == 0) {
        // true가 리턴되는 경우 v는 found에 저장하고 탐색을 중지한다.
        return true;
    } else {
