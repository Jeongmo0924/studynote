...
const GradeList = () => {
    ...
    // 수정한 대상을 백엔드로부터 로드한다. --> 자동 실행 모드
    const [{data, loading, error}, refetch] = useAxios(`http://localhost:4000/grade/${id}`);
    ...
    const onSubmit = React.useCallback((e) => {
        ...
        try {
            ...
            regexHelper.value(current.kor, '국어점수를 입력하세요.');
            regexHelper.num(current.kor, '국어점수는 숫자만 입력 가능합니다.');
            regexHelper.min(current.kor, 0, '0보다 큰 점수를 입력하세요.');
            regexHelper.max(current.kor, 100, '국어점수는 100점을 초과할 수 없습니다.');
            regexHelper.value(current.eng, '영어점수를 입력하세요.');
            regexHelper.num(current.eng, '영어점수는 숫자만 입력 가능합니다.');
            regexHelper.min(current.eng, 0, '0보다 큰 점수를 입력하세요.');
            regexHelper.max(current.eng, 100, '영어점수는 100점을 초과할 수 없습니다.');
            regexHelper.value(current.math, '수학점수를 입력하세요.');
            regexHelper.num(current.math, '수학점수는 숫자만 입력 가능합니다.');
            regexHelper.min(current.math, 0, '0보다 큰 점수를 입력하세요.');
            regexHelper.max(current.math, 100, '수학점수는 100점을 초과할 수 없습니다.');
            regexHelper.value(current.sin, '과학점수를 입력하세요.');
            regexHelper.num(current.sin, '과학점수는 숫자만 입력 가능합니다.');
            regexHelper.min(current.sin, 0, '0보다 큰 점수를 입력하세요.');
            regexHelper.max(current.sin, 100, '과학점수는 100점을 초과할 수 없습니다.');
        } catch (e) {
                ...
                data && (
                    <form onSubmit={onSubmit}>
                        <TableEx>
                            ...
                            <tr>
                                <th>이름</th>
                                <td className='inputWrapper'><input className='field' type='text' name='name'
                                                                    defaultValue={data.name}/></td>
                            </tr>
                            <tr>
                                <th>학년</th>
                            ...
                            <tr>
                                <th>성별</th>
                                <td className='inputWrapper'>
                                    <label><input type='radio' name='sex' value='남자'
                                                  defaultChecked={data.sex === '남자'}/>남자</label>
                                    <label><input type='radio' name='sex' value='여자'
                                                  defaultChecked={data.sex === '여자'}/>여자</label>
                                </td>
                            </tr>
                            <tr>
                                <th>국어</th>
