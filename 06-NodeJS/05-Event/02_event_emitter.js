...
class Radio extends EventEmitter {
  // 클래스의 상속이 이루어졌으므로, 생성자에서 상위 클래스의 생성자를 호출하도록 지정. --> 상속 구현
  // new 키워드에 의해서 호출되기 전까지는 생성자는 실행되지 않는다.
  constructor() {
    super();
  }
}
...
for (var i = 0; i < 2; i++) {
  ...
  radio.emit('turnon', i+1);
  radio.emit('changechannel', i+1);
  // once로 이벤트가 정의되었으므로 한 번만 실행된다.
  radio.emit('turnoff', i+1);
  console.debug();
  console.groupEnd();
}
