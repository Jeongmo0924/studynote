...
import serveStatic from "serve-static"; // 특정 폴더의 파일을 URL로 노출시킬
...
/*----------------------------------------------------------------------
 | 2) Express 객체 생성
 -----------------------------------------------------------------------*/
// 여기서부터 생성한 app 객체의 use()함수를 사용해서
...
/*----------------------------------------------------------------------
 | 4) Express 객체의 추가 설정
 -----------------------------------------------------------------------*/
/** POST 파라미터 수신 모듈 설정. 추가되는 미들웨어들 중 가장 먼저 설정해야 한다. */
// body-parser를 이용해 application/x-www-form-urlencoded 파싱
// extended: true --> 지속적 사용.
// extended: false --> 한 번만 사용
...
// HTML 폼에서 PUT, DELTE로 전송할 경우 post방식을 사용하되, action 주소에 "?_method"라고 추가
...
router
    ...
    .delete("/product/:productNumber", (req, res, next) => {
        // 삭제할 상품의 일련번호 전송
        const html = "<h1><span style='color:#0066ff'>" + req.params.productNumber + "</span>번 상품 <span style='color:#f00f0f'> 삭제 </span>하기 </h1>";
        res.status(200).send(html);
    });
...
router
    ...
    .delete("/cookie", (req, res, next) => {
        // 저장시 domain, path를 설정했다면, 삭제시에도 동일한 값을 저장해야 함
...
router
    .post("/session/login", (req, res, nexr) => {
    ...
    .get("/session/login", (req, res, next) => {
        ...
        const json = { rt: result_msg, id: userid, pw: userpw };
        res.status(result_code).send(json);
    });
